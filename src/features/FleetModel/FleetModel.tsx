...
import { FleetModelDialog } from "@/components/FleetModelDialog";

export function FleetModel() {
  const {
    fleetModels,
    loadMore,
    isLoading,
  ...
  const columns = columnsConfig({
    closeDialog,
    openDialog,
    handleDelete: handleDeleteFleetModel,
    isLoadingDelete,
  });
  ...
  return (
    <MainContainer>
      <AppBar>
        <HeaderTitle>Modelos de Frota</HeaderTitle>
      </AppBar>
      <Box
        sx={{
          width: "1400px",
            ...
            <div style={{ height: "100%", width: "100%" }}>
              <DataGrid
                ...
                onRowDoubleClick={(params) => {
                  handleEditFleetModel(params.id as string);
                }}
                initialState={{
                  pagination: {
      ...
      <FleetModelDialog open={!!fleetModelId} onClose={handleClose} />
      <FleetModelDialog open={!!isToAddFleetModel} onClose={handleClose} />
    </MainContainer>
  );
}
